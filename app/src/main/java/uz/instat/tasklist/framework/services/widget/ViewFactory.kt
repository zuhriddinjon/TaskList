...
class ViewFactory internal constructor(private val mContext: Context) : RemoteViewsService.RemoteViewsFactory {
    private lateinit var mWidgetData: ArrayList<TaskLocal>
...
//        mWidgetData.addAll()
    ...
    override fun getViewAt(position: Int): RemoteViews {
        ...
        if (!mWidgetData[position].description.isEmpty()) {
        ...
        if (mWidgetData[position].time != 0L) {
            remoteViews.apply {
            ...
            when {
                // Today
                DateUtils.isToday(mWidgetData[position].time) -> {
                    remoteViews.apply {
                        setTextColor(R.id.tvItemDate, ContextCompat.getColor(mContext, R.color.blue))
                        setTextViewText(R.id.tvItemDate, mWidgetData[position].time.date)
                    }
                }

                // Yesterday
                DateUtils.isToday(mWidgetData[position].time + DateUtils.DAY_IN_MILLIS) -> {
                    remoteViews.apply {
                ...
                // Far past
                mWidgetData[position].time < Calendar.getInstance().timeInMillis -> {
                    remoteViews.apply {
                        setTextColor(R.id.tvItemDate, ContextCompat.getColor(mContext, R.color.red))
                        setTextViewText(R.id.tvItemDate, mWidgetData[position].time.date)
        ...
        val fillInIntent = Intent()
            ...
            .putExtra(WidgetProvider.ITEM_NOTE, mWidgetData[position].description)
            .putExtra(WidgetProvider.ITEM_POSITION, position)
            .putExtra(WidgetProvider.ITEM_TIME_STAMP, mWidgetData[position].time)

        if (mWidgetData[position].time != 0L) {
            fillInIntent.putExtra(WidgetProvider.ITEM_DATE, mWidgetData[position].time)
        }
